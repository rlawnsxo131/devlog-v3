...
말그대로 ``대용량 트래픽 처리`` 에 대한 부분을 꼭 고려해야 합니다. 물론 ```FrontServer``` 가 ``Backend API Server`` 에 비해 수평 확장에 대해 대체적으로 유리하다는 것은 맞다란 생각입니다. 허나, 이부분은 정적 페이지로 서비스 하고, 이부분은 나중에 데이터를 페칭하도록 해서 CSR 로 화면을 그리도록 하자. 이부분은 동적으로 바뀌어야 하는 데이터가 있지만 완전히 실시간 성일 필요는 없으니, 캐시를 도입해서 부하를 줄이자 라는 등의 의사결정을 위한 논의가 필요합니다. 아울러 팀원들이 이에대한 경험이 없다면 섯부르게 도입했다가 오히려 더 큰 폭탄과 함께 할 수도 있는 부분이란 생각입니다.
