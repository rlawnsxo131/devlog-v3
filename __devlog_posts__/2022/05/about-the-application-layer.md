...
```
실습편에서 조금더 자세히 언급하겠지만 추상화의 수준이 높은경우와 반대의 경우는 장단점이 분명히 존재합니다. 이를 상황에 따라, 혹은 어플리케이션의 특성, 팀의 특성에 따라 선택하는 것은 결국엔 경험의 영역인지라 후에 실습을 진행하며 조금더 자세히 언급하겠습니다.
...
실습편에서 언급할 내용을 살짝 미리 말씀드리자면, 저 두가지 행위가 묶인 개념의 행동을 정의하는 함수는 이후 공통적인 관심사로 이루어지는 구간이 굉장히 반복이 된다할때 떼어내는것을 추천하고 싶습니다.
...
다음 편에서는 리얼월드에 가까운 예시를 직접 구현하며 여태 정리해온 개념을 코드로 만나보는 시간을 갖을 텐데요. 이번엔 코드를 github 에 함께 올릴 예정이니 조금만 기다려 주세요!
제가 글을 작성하기 전에, 혹은 이후라도 보고싶은 예시상황이 있다면 댓글에 남겨주세요. 감사히 참고하고, 해당 사항을 구현한 예제를 작성토록 고려하겠습니다. 감사합니다.